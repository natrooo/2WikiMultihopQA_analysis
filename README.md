...
2WikiMultihopQA 是一个面向多跳推理的大规模问答数据集，由 Ho 等人在 2021 年 EMNLP 会议上提出。与 HotpotQA 类似，该数据集要求模型跨越两篇维基百科文章进行推理才能得出正确答案。
...
| 可选扩展 | DuckDB + hf:// parquet | 对全量数据做 SQL 分析 |

选择 SQLite 的原因：单文件、零运维、支持 FTS5 与关系查询，适合 2WikiMultihopQA 的「问题—段落—支撑事实」层级结构。
...
├── scripts/
│   ├── import_data.py           # HF API → SQLite
│   └── export_web_data.py       # SQLite → JSON 静态文件
